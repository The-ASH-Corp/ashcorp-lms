...
export default function CreateChapter() {
  const [courseTitle, setCourseTitle] = useState('Advance Adobe Illustrator Mastery Course');
  const [chapterTitle, setChapterTitle] = useState('');
  const [serial, setSerial] = useState('9');
  const [isFreeContent, setIsFreeContent] = useState(false);
  const [contentItems, setContentItems] = useState<ContentItem[]>([
    { id: 1, title: '', sequence: 1, fileType: 'VideoAsset', fileName: null },
  ]);
  ...
  const updateContentItem = (id: number, field: string, value: any) => {
    setContentItems(
      contentItems.map((item) => (item.id === id ? { ...item, [field]: value } : item))
    );
  };
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-6 py-4 bg-gray-50 border-b border-gray-200">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Free Content</span>
            </label>
          </div>
                  ...
                  <tr key={item.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 sm:px-6 py-4 hidden sm:table-cell">
                      <input
                        type="number"
                        value={item.sequence}
                        onChange={(e) => updateContentItem(item.id, 'sequence', e.target.value)}
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="relative">
                        ...
                        >
                          <option>VideoAsset</option>
                          <option>AudioAsset</option>
                          <option>DocumentAsset</option>
                          <option>ImageAsset</option>
                        </select>
                        <ChevronDown className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none" size={16} />
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4">
                      <button className="flex items-center gap-2 px-3 py-2 border border-gray-300 rounded text-gray-700 hover:border-gray-400 transition-colors text-sm whitespace-nowrap">
                        <Plus size={16} />
                        Choose
                      </button>
                    </td>
