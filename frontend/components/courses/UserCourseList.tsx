          ...
          {course.status === 'completed' && (
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Link
                ...
                className="px-6 py-2 bg-primary text-white rounded-lg font-semibold hover:bg-violet-700 transition-colors"
              >
                View Certificate
              </Link>
              ...
              <Link
                href={`/play?courseId=${course.id}`}
                className="flex items-center gap-1 p-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
              >
                <RefreshCw size={18} className="text-gray-700" /> Rewatch
              </Link>
            </div>
          )}
        </div>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow h-full flex flex-col">
      ...
      <div className="p-4 sm:p-6 flex-1 flex flex-col">
        ...
        <p className="text-sm text-gray-600 mb-4 flex items-center gap-1">👤 {course.instructor}</p>
