...
export function InstructorForm({
  ...
  const handleToggle = (name: keyof Pick<FormData, "isFeatured" | "verifyByDefault">) => {
    setFormData((previous) => ({
      ...previous,
      [name]: !previous[name],
    }));
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (errors.profileImage) {
        setErrors((previous) => ({ ...previous, profileImage: undefined }));
      }
    }
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const schema = mode === "edit" ? updateInstructorSchema : createInstructorSchema;
    ...
    try {
      if (mode === "edit" && instructorId) {
        const result = await updateInstructor({ id: instructorId, instructor: payload }).unwrap();
        toast.success(result?.name ? "Instructor updated successfully" : "Instructor updated successfully");
      } else {
      ...
      toast.error(mode === "edit" ? "Instructor update failed" : "Instructor creation failed");
    }
  };
        ...
        <form className="space-y-4 sm:space-y-6 lg:space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="space-y-6">
              <div>
                ...
                {errors.instructorTitle ? (
                  <p className="text-xs text-red-500 mt-2">{errors.instructorTitle}</p>
                ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">
                    Profile Picture (JPG, JPEG, PNG) {mode === "create" && <span className="text-red-500">*</span>}
                  </label>
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 sm:p-8 text-center hover:border-primary transition-colors cursor-pointer">
                    ...
                    <label htmlFor="profile-upload" className="cursor-pointer block">
                      ...
                      <p className="text-gray-600 text-sm font-medium mb-1">
                        Drag and drop or <span className="text-primary font-bold">Choose a file</span>
                      </p>
                      <p className="text-xs text-gray-500">RECOMMENDED SIZE: 800x800px</p>
                    </label>
                  </div>
                  {profileImage && <p className="text-xs text-green-600 mt-2">✓ {profileImage.name}</p>}
                  {errors.profileImage && <p className="text-xs text-red-500 mt-2">{errors.profileImage}</p>}
                </div>
                ...
                <div>
                  ...
                  {errors.about ? (
                    ...
                    <p className="text-xs text-gray-500 mt-2">0/500 characters</p>
                  )}
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={handleInputChange}
                      placeholder={mode === "edit" ? "Leave blank to keep current password" : "Enter user password"}
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={handleInputChange}
                      placeholder={mode === "edit" ? "Leave blank to keep current password" : "Enter user password again"}
                      className="w-full h-12 px-4 py-3 pr-12 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-violet-100 outline-none transition-colors"
                    />
                    <button
                      type="button"
              ...
              <div className="space-y-4 flex gap-3 justify-evenly">
                <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg gap-3">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.isFeatured ? "bg-primary" : "bg-gray-300"}`}
                  >
                ...
                <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg h-fit gap-3">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.verifyByDefault ? "bg-primary" : "bg-gray-300"}`}
                  >
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-4">
            <button
              ...
              className="px-8 py-3 bg-primary text-white rounded-lg hover:bg-violet-700 font-medium transition-colors disabled:opacity-60"
            >
              {isCreating || isUpdating ? (mode === "edit" ? "Updating..." : "Creating...") : mode === "edit" ? "Update Instructor Profile" : "Create Instructor Profile"}
            </button>
          </div>
        </form>
      </div>
