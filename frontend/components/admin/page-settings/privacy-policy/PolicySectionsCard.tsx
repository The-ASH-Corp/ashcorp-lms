...
interface PolicySectionsCardProps {
  policySections: IPrivacyPolicySettings["policySections"];
  isVisible?: boolean;
  onToggleVisibility?: () => void;
  onChange: (updated: IPrivacyPolicySettings["policySections"]) => void;
}

export function PolicySectionsCard({
  policySections,
  isVisible = true,
  onToggleVisibility,
  onChange,
}: PolicySectionsCardProps) {
  const items = policySections?.items || [];
  ...
  const handleAddSection = () => {
    ...
    const newItem: IPolicySectionItem = {
      id: `pol-${Date.now()}`,
          ...
          <div className="rounded-2xl border border-violet-200 bg-white p-4 space-y-3 shadow-xs">
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 text-xs outline-none focus:border-violet-600 focus:ring-2 focus:ring-violet-600/10"
                placeholder="Describe the policy details for this section..."
              />
            </div>
