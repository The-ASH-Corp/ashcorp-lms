...
export class MongoChapterRepository implements ChapterRepository {
  ...
  async getPaginatedChaptersByCourseId(
    courseId: string,
    page: number,
    limit: number,
    searchTerm?: string,
  ): Promise<{ chapters: Chapter[]; totalChapters: number }> {
    ...
    const [chapters, totalChapters] = await Promise.all([
      ChapterModel.find(searchFilter)
    ...
    return {
      chapters: chapters.map(
        (chapter) => chapter.toObject() as unknown as Chapter,
      ),
      totalChapters,
    };
  }
  ...
  async updateChapter(
    id: string,
    data: Partial<ChapterRequestDTO>,
  ): Promise<Chapter | null> {
    const chapter = await ChapterModel.findByIdAndUpdate(id, data, {
      new: true,
    });
    return chapter ? (chapter.toObject() as unknown as Chapter) : null;
  }
